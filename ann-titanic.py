# -*- coding: utf-8 -*-
"""
Created on Wed Sep 16 23:43:09 2020
...
X1= pd.get_dummies(train, columns =['Embarked','Sex'] , drop_first = True)
train = X1

#imputing the age column in train and test set
from sklearn.impute import SimpleImputer
age_imputer = SimpleImputer(missing_values= np.nan , strategy='mean', fill_value=None, verbose=0, copy=True)
...
#imputing the Fare column in test set
from sklearn.impute import SimpleImputer
fare_imputer = SimpleImputer(missing_values= np.nan , strategy='mean', fill_value=None, verbose=0, copy=True)
fare_imputer.fit(train['Fare'].values.reshape(-1, 1))
test['Fare'] = fare_imputer.transform(test['Fare'].values.reshape(-1, 1))

X = train.iloc[:, [0,2,3,4,5,6,7,8,9]]
y = train.iloc[:, 1]
...
# =============================================================================
ann.add(tf.keras.layers.Dense(units=9,kernel_initializer='uniform', activation='relu'))
...
def build_classifer():
    ...
    ann.compile(optimizer = 'rmsprop', loss = 'binary_crossentropy', metrics = ['accuracy'])
    return ann
ann = KerasClassifier(build_fn = build_classifer, batch_size = 10, nb_epoch = 100)
